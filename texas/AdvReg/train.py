...
def main():
    ...
    parser.add_argument('--classifier_epochs', type = int, default = 20, help = 'classifier epochs')
    ...
    attack_optimizer0 = optim.Adam(attack_model0.parameters(),lr=0.001)
    ...
    for epoch in range(1, classifier_epochs+1):
        ...
        if epoch == 1:
            ...
            for i in range((len(train_data)//(batch_size*1))):
                _, at_acc = privatly_attack(train_data_tensor, train_label_tensor, ref_data_tensor, ref_label_tensor, model, attack_model0, attack_criterion0, attack_optimizer0, batch_size, 50, (i*50)%((len(train_data)//batch_size)))
                _, _ = train_privatly(train_data_tensor, train_label_tensor, model, attack_model0, criterion, optimizer, batch_size, 2, (i*2)%((len(train_data)//batch_size)), alpha)
