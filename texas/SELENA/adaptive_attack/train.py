...
def main():
    parser = argparse.ArgumentParser(description = 'Setting for Texas datataset adaptive attack')
    ...
    parser.add_argument('--split_epochs', type = int, default = 30, help = 'training epochs for each single model in split-ai')
    ...
    for i in range(split_model):
        ...
        for ind in range(len(attack_know_data)):
            tmp_ind = attack_know_data[ind, -non_model:]
            if i not in tmp_ind:
                split_train_data_list.append(attack_know_data[ind,:-non_model])
                split_train_label_list.append(attack_know_label[ind])
            else:
                split_test_data_list.append(attack_know_data[ind,:-non_model])
                split_test_label_list.append(attack_know_label[ind])
        ...
        #print first 50 labels for each subset i in shadow splitai, for later checking
        ...
        for epoch in range(split_epochs):
            ...
            if split_is_best:
               split_save_checkpoint({
                ...
                }, split_is_best, split_best_acc, split_name = str(i), checkpoint = checkpoint_path_shadow)
